...
func newLogsCommand(dockerCli *command.DockerCli) *cobra.Command {
	...
	flags := cmd.Flags()
	// these options are specific to docker service logs
	flags.BoolVar(&opts.noResolve, "no-resolve", false, "Do not map IDs to Names in output")
	flags.BoolVar(&opts.noTrunc, "no-trunc", false, "Do not truncate output")
	flags.BoolVar(&opts.pretty, "pretty", true, "Neatly format the log context before printing")
	flags.BoolVar(&opts.noTaskIDs, "no-task-ids", false, "Do not include task IDs in output")
	// these options are also found in container logs
...
func runLogs(dockerCli *command.DockerCli, opts *logsOptions) error {
	...
	options := types.ContainerLogsOptions{
		...
		Follow:     opts.follow,
		Tail:       opts.tail,
		// get the details if we request it OR if we're doing pretty mode
		// (we need them for the context)
		Details: opts.details || opts.pretty,
	}
	...
	var (
		...
		tty          bool
		// lf is used to delay the call to logs so that we can do some
		// processing before we actually get the logs
		lf func(context.Context, string, types.ContainerLogsOptions) (io.ReadCloser, error)
	)
	...
	if err != nil {
		...
		if tty && opts.pretty {
			return errors.New("pretty=true not supported with TTY logs")
		}
	...
	if tty && opts.pretty {
		return errors.New("tty service logs only supported with --pretty=false")
	}

	// after we've verfied that we're not TTY AND pretty is false, then we get
	// the logs
	responseBody, err = lf(ctx, opts.target, options)
	if err != nil {
		return err
	}
	...
	// otherwise, logs are multiplexed. if we're doing pretty, also create a
	// task formatter.
...
type logWriter struct {
	...
	// detailsCache saves a pre-sorted list of the details keys, so we don't
	// have to recompute their order every time. this operates under the
	// assumption that the details keys are always the same (they should be)
	detailsCache []string
}
...
func (lw *logWriter) Write(buf []byte) (int, error) {
	...
	if lw.opts.details {
		...
		// check if the detailsCache is filled yet. if not, make one.
		if lw.detailsCache == nil {
			// first we need a backing array
			d := make([]string, 0, len(details))
			// then let's add all the keys
			for k, _ := range details {
				d = append(d, k)
			}
			// then sort em
			sort.Strings(d)
			// and then save the cache
			lw.detailsCache = d
		}
		d := make([]string, 0, len(details))
		for _, k := range lw.detailsCache {
			d = append(d, fmt.Sprintf("%v=%v", k, details[k]))
		}
