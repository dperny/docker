...
func (c *Cluster) ServiceLogs(ctx context.Context, selector *backend.LogSelector, config *backend.ContainerLogsConfig) (rerr error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	// if we return nil, it means we're still using this channel, so don't close
	// it
	defer func() {
		if rerr != nil {
