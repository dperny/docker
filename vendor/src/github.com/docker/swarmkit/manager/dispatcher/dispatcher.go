		...
		for _, n := range nodes {
			err := batch.Update(func(tx store.Tx) error {
				...
				expireFunc := func() {
					...
					if err := d.markNodeNotReady(nodeID, api.NodeStatus_DOWN, `heartbeat failure for node in "unkonwn" state`); err != nil {
...
func (d *Dispatcher) markNodeReady(nodeID string, description *api.NodeDescription, address string) error {
...
func (d *Dispatcher) register(ctx context.Context, nodeID string, description *api.NodeDescription) (string, error) {
	...
	address, err := nodeIPFromContext(ctx)
	if err != nil {
		log.G(ctx).Debugf(err.Error())
	}

	if err := d.markNodeReady(nodeID, description, address); err != nil {
		return "", err
	}
...
func (d *Dispatcher) markNodeNotReady(id string, state api.NodeStatus_State, message string) error {
	...
	// TODO(dperny) lock before or after we view the node?
	d.nodeUpdatesLock.Lock()

	// lookup the node so we can get its old address
	// TODO(dperny) is there a faster/better way to do this?
	var node *api.Node
	d.store.View(func(tx store.ReadTx) {
	...
	// TODO(dperny) explain why we can just pluck the description out of nodeUpdates?
...
func (d *Dispatcher) Session(r *api.SessionRequest, stream api.Dispatcher_SessionServer) error {
	...
	if _, err := d.nodes.GetWithSession(nodeID, r.SessionID); err != nil {
		...
		address, err := nodeIPFromContext(stream.Context())
		if err != nil {
			log.G(ctx).Debugf(err.Error())
		}
		// update the node description
		if err := d.markNodeReady(nodeID, r.Description, address); err != nil {
			return err
		}
	}
